...
while len(i_population_unique) < pop_size:
    ...
    [i_population_unique,_] = remove_duplicate_same_population(
                                                        i_population)
...
# a risk vector (i_chrom_risk), having length equal to i_chrom_cost

# for demonstration, hypothetical risk is generated for different chromosomes
i_chrom_risk = np.random.randint(100, size = (pop_size, 1))
...
for generation in range(1,4):
    ...
    ## To rank the individual chromosomes
    p_front = non_dominated_sorting(pop_size,p_chroms_objs)
    
    ## To find crowding distance
    ...
    while len(offspring) < pop_size and c < 5000:        
        ...
        # compare offspring population with parent population to remove
        # duplicates (if any)
    ...
    o_chrom_risk = np.random.randint(100, size = (pop_size, 1))
